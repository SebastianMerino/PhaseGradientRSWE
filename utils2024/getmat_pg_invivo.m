function [Ax_large, Az_large, bx_large, bz_large, size_out] = getmat_pg_invivo(u, w_kernel, dinf, og_size, stride)
...
% 
% Inputs:  
%          u           : 3D region of interest to evaluate (previously mirror padding)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    size_mirror = size(extended_u, [1,2]); % ogsize + w_kernel - 1; %  = size (u)
    ...
    for jj = 1:st:og_size(2)
        ...
        for ii = 1:st:og_size(1) %% for faster computing pararell toolbox
            ...
            resz_small = Az_small\bz_small; % 2 channels [kz phi0]
            resx_small = Ax_small\bx_small; % 2 channels [kx phi0]
    ...
    elemA = reshape(1:1*numSubMatrices,[1,numSubMatrices]);
    elemA = elemA';
    icolA = elemA(:);
